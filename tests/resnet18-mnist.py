...
class ResNet18(pl.LightningModule):
    def __init__(self,input_dim, n_classes, outpath = "." ):
        super().__init__()
...
class vggnet(pl.LightningModule):
    def __init__(self, input_dim, n_classes, outpath="."):
        super().__init__()
        self.model = vgg16(n_classes)
        self.model.conv1 = nn.Conv2d(1, 64, kernel_size=3, padding=1, stride = 1, bias=True)
        # self.loss = nn.CrossEntropyLoss()
        self.outpath = outpath
        self.input_dim = input_dim
        self.n_classes = n_classes
...
"""
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.RMSprop(self.parameters(), lr=0.005)
        return optimizer
    ...
    def fit(self, X, y):
        ...
        train_dataloader = DataLoader(TensorDataset(XTrainT, YTrainT), batch_size=64, num_workers=20)
        val_loader = None
...
def main():
    ...
    parser.add_argument('--dataset', '-d', required=True,
                        help='Dataset to to be downloaded and used. Currently supported are {mnist, fashion}.')
    parser.add_argument("--binarize", "-b", required=False, action='store_true',
                        help="Trains a binarized neural network if true.")
    ...
    if args.dataset in ["mnist", "fashion"]:
        ...
        print(
            "Only {eeg, magic, mnist, fashion} is supported for the --dataset/-d argument but you supplied {}.".format(
                args.dataset))
        sys.exit(1)

    model = Resnet18(n_features, n_classes, args.outpath)
    ...
    performance = test_implementations(model=model, dataset=args.dataset, split=args.split,
                                       implementations=implementations, base_optimizers=optimizers,
                                       out_path=args.outpath, model_name=args.modelname)
