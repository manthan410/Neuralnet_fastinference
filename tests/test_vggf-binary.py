...
class VGG(pl.LightningModule):
    ...
    def __init__(self, input_dim, n_classes, binarize = False, outpath = "."):
        ...
        if binarize:
            ...
            self.fc_1 = BinaryLinear(28*28*2, 1024)
...
class VGGNet(pl.LightningModule):
    def __init__(self, input_dim, input_size = (1,28,28), n_channels = 32, depth = 4, hidden_size = 512, p_dropout = 0.0, n_classes = 10, outpath = "."):
        super().__init__()
        ...
        def make_layers(level, n_channels):
            return [
                BinaryConv2d(in_channels if level == 0 else level*n_channels, (level+1)*n_channels, kernel_size=3, padding=1, stride = 1, bias=True),
                nn.BatchNorm2d((level+1)*n_channels),
                BinaryTanh(),
                BinaryConv2d((level+1)*n_channels, (level+1)*n_channels, kernel_size=3, padding=1, stride = 1, bias=True),
                nn.BatchNorm2d((level+1)*n_channels),
                BinaryTanh(),
        ...
        model.extend(
            [
                Flatten(),
                BinaryLinear(lin_size, hidden_size),
                nn.Dropout(p_dropout) if p_dropout > 0 else None,
                nn.BatchNorm1d(hidden_size),
                BinaryTanh(),
                BinaryLinear(hidden_size, n_classes),
            ]
	    )
    ...
    def fit(self, X, y):
        ...
        train_dataloader = DataLoader(TensorDataset(XTrainT, YTrainT), batch_size=256, num_workers=15)
        val_loader = None 

        trainer = pl.Trainer(max_epochs = 150, default_root_dir = self.outpath, enable_progress_bar=True)
