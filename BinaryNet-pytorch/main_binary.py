...
def main():
    ...
    #checkpoint = torch.load('results/Vgg-test-cifar10/checkpoint.pth.tar') #, map_location=torch.device('cuda')
    #model.load_state_dict(checkpoint['state_dict'], strict=False)
    dummy_x = torch.randn(1, 3072, requires_grad=False, device="cuda")
    model = sanatize_onnx(model)
    #onnx_path = os.path.join(results, args.model + ".onnx")
    torch.onnx.export(model, dummy_x, 'vgg4-cifar10.onnx', training=torch.onnx.TrainingMode.PRESERVE, export_params=True,
                      opset_version=12, do_constant_folding=True, input_names=['input'], output_names=['output'],
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        if not training:
            with torch.no_grad():
                ...
                #input_var = Variable(inputs.type('torch.FloatTensor'), volatile=not training)
